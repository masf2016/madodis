...
$app->get('/coordenadores', function ($request, $response) {
   $query = 'SELECT * FROM tb_coordenador';
    try {
        $db = getConnection();
        $stmt = $db->query($query);
...
//CONSULTAR UM COORDENADOR ESPECÍFICO
$app->get('/coordenador/{id}', function ($request, $response, $arguments) {
    $query = 'SELECT * FROM tb_coordenador WHERE id_coord=:id';
    try {
        $db = getConnection();
        $stmt = $db->prepare($query);
        $stmt->bindParam('id_coord', $arguments['id_coord'], PDO::PARAM_INT);
        $stmt->execute();
...
//DELETAR UM ALUNO
$app->delete('/coordenador/{id}', function ($request, $response, $arguments) {
    $query = 'DELETE FROM tb_coordenador WHERE id_coord=:id';
    try {
        $db = getConnection();
        $stmt = $db->prepare($query);
        $stmt->bindParam('id_coord', $arguments['id_coord'], PDO::PARAM_INT);
...
//INSERIR COORDENADOR
$app->post('/coordenador', function ($request, $response) {
    $dadoInsert = $request->getParsedBody();
    $query = 'INSERT INTO tb_coordenador (nome,
                                  sexo,
                                  datanascim,
                                  nomepai,
                                  nomemae,
    ...
    try {
        ...
        $stmt->bindParam('emexercicio', $dadoInsert['emexercicio'] );
        $stmt->execute();
        $dadoInsert['id_coord'] = $db->lastInsertId();
        $db = null;
...
$app->put('/coordenador/{id}', function ($request, $response, $arguments) {
    $dadoUpdate = $request->getParsedBody();
    $query = 'UPDATE tb_coordenador SET nome=:nome,
                                sexo=:sexo,
                                ...
                                emexercicio:=emexercicio WHERE id_coord=:id';
    try {
        $db = getConnection();
        $stmt = $db->prepare($query);
        ...
        $stmt->bindParam('emexercicio', $dadoUpdate['emexercicio'] );
        $stmt->bindParam('id_coord', $arguments['id_coord'], PDO::PARAM_INT);
        $stmt->execute();
        $db = null;
