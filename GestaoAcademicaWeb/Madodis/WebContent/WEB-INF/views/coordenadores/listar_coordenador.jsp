...
<title>Madodis - Listar Coordenadores</title>
...
<body>
	...
	<jsp:include page="../menu.jsp">
		...
		<div class="container">
			<h3>
				<legend>
					<a href="/madodis/inicio">Coordenador Cadastrado</a>
				</legend>
			</h3>
		</div>
		<c:if test="${empty coordenadorList}">
			<div class="container">
				<div class="col-mod-4">
					<h4>Não existe registro de coordenadores para mostrar!</h4>
				</div>
			</div>
		</c:if>
		<div class="container">
			...
			<div class="col-mod-4">
				<table class="table table-striped">
					...
					<c:forEach items="${coordenadorList}" var="coordenador">
						<tr>
							<td>${coordenador.nome}</td>
							<td>${coordenador.cpf}</td>
							<td>${coordenador.matricula}</td>
							<td><a href="javascript:void(0)" onclick="removerCoordenador(${coordenador.id})"><span
			...
			<div>
				<a href="/madodis/form_coordenadores"><button class="btn btn-success">
		...
		<script>
			function removerCoordenador(id) {
				$.jAlert({
			        'title': 'Remover',
			        'content': 'Voce deseja remover o coordenador?',
			        'theme': 'blue',
			        'btns': [
			        			{ 
			        				'text': 'Sim', 
			        				'onClick' : function(e, btn) {
			        					$.get("/madodis/remover/coordenador/" + id, function(data) {
			        						document.open();
			        						document.write(data);
			        						document.close();
			        					});
