...
<title>Madodis - Listar Turmas</title>
...
<body>
	...
	<jsp:include page="../menu.jsp">
		...
		<div class="container">
			<h3>
				<legend>
					<a href="javascript:void(0)">Turma Cadastrada</a>
				</legend>
			</h3>
		</div>
		<c:if test="${empty turmaList}">
			<div class="container">
				<div class="col-mod-4">
					<h4>Não existe registro de turmas para mostrar!</h4>
				</div>
			</div>
		</c:if>
		<div class="container">
			...
			<div class="col-mod-4">
				<table class="table table-striped">
					...
					<c:forEach items="${turmaList}" var="turma">
						<tr>
							<td>${turma.nome}</td>
							<td>${turma.turmaLetra}</td>
							<td>${turma.turno}</td>
							<td>${turma.disciplina.nome}</td>
							<td><a href="javascript:void(0)" onclick="removerTurma(${turma.id})"><span
									class="glyphicon glyphicon-remove-circle" title="Remover"></span></a></td>
							<td><a href="/madodis/update/turma/${turma.id}"><span 
			...
			<div>
				<a href="/madodis/form_turmas"><button class="btn btn-success">
		...
		<script>
			function removerTurma(id) {
				$.jAlert({
			        'title': 'Remover',
			        'content': 'Voce deseja remover a turma?',
			        'theme': 'blue',
			        'btns': [
			        			{ 
			        				'text': 'Sim', 
			        				'onClick' : function(e, btn) {
			        					$.get("/madodis/remover/turma/" + id, function(data) {
			        						document.open();
			        						document.write(data);
			        						document.close();
			        					});
