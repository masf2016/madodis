...
<title>Madodis - Listar Matricula</title>
...
<body>
	...
	<jsp:include page="../menu.jsp">
		...
		<div class="container">
			<h3>
				<legend>
					<a href="javascript:void(0)">Matricula Cadastrada</a>
				</legend>
			</h3>
		</div>
		<c:if test="${empty matriculaList}">
			<div class="container">
				<div class="col-mod-4">
					<h4>Não existe registro de matricula de alunos para mostrar!</h4>
				</div>
			</div>
		</c:if>
		<div class="container">
				...
				<table class="table table-striped">
					...
					<c:forEach items="${matriculaList}" var="matricula">
						<tr>
							...
							<td>${matricula.turma.turmaLetra}</td>
							<td>${matricula.turma.turno}</td>
							<td>${matricula.turma.disciplina.nome}</td>
							<td><a href="javascript:void(0)" onclick="removerMatricula(${matricula.id})"><span
									class="glyphicon glyphicon-remove-circle" title="Remover"></span></a></td>
							<td><a href="/madodis/update/matricula/${matricula.id}"><span 
			...
			<div>
				<a href="/madodis/form_matriculas"><button class="btn btn-success">
		...
		<script>
			function removerMatricula(id) {
				$.jAlert({
			        'title': 'Remover',
			        'content': 'Voce deseja remover a matricula?',
			        'theme': 'blue',
			        'btns': [
			        			{ 
			        				'text': 'Sim', 
			        				'onClick' : function(e, btn) {
			        					$.get("/madodis/remover/matricula/" + id, function(data) {
			        						document.open();
			        						document.write(data);
			        						document.close();
			        					});
