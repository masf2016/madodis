...
<body>
	...
	<jsp:include page="../menu.jsp">
		...
		<c:if test="${empty arquivos}">
			<div class="container">
				<div class="col-mod-4">
					<h4>Não existe registro de materiais para mostrar!</h4>
				</div>
			</div>
		</c:if>
		<div class="container">
				...
				<table class="table table-striped">
					...
					<c:forEach items="${arquivos}" var="arquivo">
						<tr>
							...
							<td><a href="javascript:void(0)" onclick="removerArquivoAula(${arquivo.id})"><span
		...
		<div class="container">
			...
			<div class="col-mod-4">
				...
				<form:form method="post" action="/madodis/aula/salvarArquivo" enctype="multipart/form-data">
		...
		<script>
			function removerArquivoAula(id) {
				$.jAlert({
			        'title': 'Remover',
			        'content': 'Voce deseja remover este arquivo?',
			        'theme': 'blue',
			        'btns': [
			        			{ 
			        				'text': 'Sim', 
			        				'onClick' : function(e, btn) {
			        					$.get("/madodis/remover/arquivo/" + id, function(data) {
			        						document.open();
			        						document.write(data);
			        						document.close();
			        					});
