...
<title>Madodis - Listar Discentes</title>
...
<body>
	...
	<jsp:include page="../menu.jsp">
		...
		<div class="container">
			<h3>
				<legend>
					<a href="javascript:void(0)">Discente Cadastrado</a>
				</legend>
			</h3>
		</div>
		<c:if test="${empty alunoList}">
			<div class="container">
				<div class="col-mod-4">
					<h4>Nada existe registro de discentes para mostrar!</h4>
				</div>
			</div>
		</c:if>
		<div class="container">
				...
				<table class="table table-striped">
					...
					<c:forEach items="${alunoList}" var="aluno">
						<tr>
							...
							<td><a href="javscript:void(0)" onclick="removerAluno(${aluno.id})"><span
			...
			<div>
				<a href="/madodis/form_alunos"><button class="btn btn-success">
		...
		<script>
			function removerAluno(id) {
				$.jAlert({
			        'title': 'Remover',
			        'content': 'Voce deseja remover o discente?',
			        'theme': 'blue',
			        'btns': [
			        			{ 
			        				'text': 'Sim', 
			        				'onClick' : function(e, btn) {
			        					$.get("/madodis/remover/aluno/" + id, function(data) {
			        						document.open();
			        						document.write(data);
			        						document.close();
			        					});
